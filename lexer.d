...
import util : lowerf, escape, mallocAppender, toEngNum, ToTuple;
...
string[] keywords = ["def","state","new","drop","dup","fwd","skip","assert","observe","and","not","if","then","else","true","false","or","topology","nodes","links","parameters","packet_fields","programs","with","query","num_steps","queue_capacity","FwdQ","RunSw"]~["for","in","return","post_observe"];
...
TokenType isKeyword(string s){
	switch(s){
	default: return Tok!"i";
		foreach(kw;ToTuple!keywords)
