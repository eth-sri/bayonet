...
class Builder{
	...
	class Program{
		...
		string toPRISM(string nodeName){
			...
			int fwdActUnlkVal = cast(int)nodes.length+nodeId[nodeName];
			int fwdActNextUnlkVal = (fwdActUnlkVal+1)%(2*cast(int)nodes.length);
			fwdAct ~= "[] unlk="~to!string(fwdActUnlkVal)~" & numSteps<maxNumSteps & "~mangName~"pc=0 & "~mangName~"os>0";
			UpdateList fwdStartUL;
			fwdStartUL ~= q(mangName~"pc","-5");
			fwdStartUL ~= q("unlk","-1");
			fwdStartUL ~= q("numSkips","0");
			fwdAct ~= updateListToPRISM(fwdStartUL);
			fwdAct ~= "[] unlk="~to!string(fwdActUnlkVal)~" & numSteps<maxNumSteps & numSkips<maxNumSkips & "~mangName~"pc=0 & "~mangName~"os=0";
			UpdateList fwdFailedStartUL;
			fwdFailedStartUL ~= q("unlk",to!string(fwdActNextUnlkVal));
			fwdFailedStartUL ~= q("numSkips","numSkips+1");
			fwdAct ~= updateListToPRISM(fwdFailedStartUL);
			foreach(srcNodeName, srcPorts; links){
				foreach(srcPort, dest; srcPorts){
					if(srcNodeName == nodeName){
			...
			fwdEndUL ~= q("unlk",to!string(fwdActNextUnlkVal));
			fwdEndUL ~= q("numSteps","numSteps+1");
			fwdAct ~= updateListToPRISM(fwdEndUL);
			TempVarData tvd;
			string runAct = "[] unlk="~to!string(nodeId[nodeName])~" & numSteps<maxNumSteps & "~mangName~"pc=0 & "~mangName~"is>0";
			UpdateList runStartUL;
			runStartUL ~= q(mangName~"pc","1");
			runStartUL ~= q("unlk","-1");
			runStartUL ~= q("numSkips","0");
			runAct ~= updateListToPRISM(runStartUL);
			runAct ~= "[] unlk="~to!string(nodeId[nodeName])~" & numSteps<maxNumSteps & numSkips<maxNumSkips & "~mangName~"pc=0 & "~mangName~"is=0";
			UpdateList runFailedStartUL;
			runFailedStartUL ~= q("unlk",to!string(nodeId[nodeName]+1));
			runFailedStartUL ~= q("numSkips","numSkips+1");
			runAct ~= updateListToPRISM(runFailedStartUL);
			int instCtr=1;
			for(int i=0; i<stms.length; ++i){
				if(!stms[i])
					continue;
			...
			runEndUL ~= q("unlk",to!string(nodeId[nodeName]+1));
	...
	string toPRISM(){
		...
		finishedCondition = "numSkips=maxNumSkips"~nodes.map!(n=>" & "~mangleNode(n)~"is=0 & "~mangleNode(n)~"os=0").join();
